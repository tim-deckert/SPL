        ...
        => val etp = typecheck(er)
           if (op.equals("not"))
              ...
              if(typeEquivalence(etp, IntType()) || typeEquivalence(etp, FloatType()))
                 etp
              else error("UMINUS operation can only be applied to Int of Float: "+e)
           else error("Not a unary op")

      case IntConst (v) 
        => val vt = typecheck(v)
           if (typeEquivalence(vt, IntType()))
              vt
           else error("Not Int const type"+e)

      case FloatConst (v) 
        => val vt = typecheck(v)
           if (typeEquivalence(vt, FloatType()))
              vt
           else error("Not Float const type"+e)

      case StringConst (v)
        => val vt = typecheck(v)
           if (typeEquivalence(vt, StringType()))
              vt
           else error("Not String const type"+e)

      case BooleanConst (v)
        => val vt = typecheck(v)
           if (typeEquivalence(vt, BooleanType()))
              vt
           else error("Not Boolean const type"+e)
